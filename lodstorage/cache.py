...
from lodstorage.yamlable import lod_storable, YamlAble
...
class Cache:
    ...
    def last_accessed(self)->datetime:
        cache_last_accessed = datetime.fromtimestamp(os.path.getmtime(self.path)) if os.path.isfile(self.path) else None
...
class CacheManager:
    ...
    def load(self, 
             ...
             count_attr:str=None) -> Union[List, Dict, None]:
        """
        ...
        if cache.is_stored:
            if ext == ".json":
                if cls and hasattr(cls, "load_from_yaml_file"):
                    result = cls.load_from_json_file(cache.path)  # Adjusted for class method
                else:
                    with open(cache.path) as json_file:
                        result = orjson.loads(json_file.read())
            elif ext == ".yaml":
                if cls and hasattr(cls, "load_from_yaml_file"):
                    result = cls.load_from_yaml_file(cache.path)  # Adjusted for class method
                else:
                    raise ValueError("YAML deserialization requires a cls parameter that is a subclass of YamlAble.")
            else:
        ...
        if ext == ".json":
            ...
            if isinstance(data_to_store,list):
                ...
                if hasattr(data_to_store, "save_to_json_file"):
                    ...
                    raise ValueError("JSON serialization requires a 'save_to_json_file' method")
        elif ext == ".yaml":
            if hasattr(data_to_store, "save_to_yaml_file"):
                ...
                raise ValueError("YAML serialization requires a 'save_to_yaml_file' method.")
        else:
