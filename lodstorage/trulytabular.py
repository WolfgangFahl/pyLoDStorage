...
{{
  ...
  ?item wdt:P31 wd:{self.item.qid}.{self.where}
    ...
    def generateSparqlQuery(self,lang:str='en',naive:bool=True)->str:
        '''
...
# generated by {script} on {self.isodate}
PREFIX wd: <http://www.wikidata.org/entity/>
PREFIX wdt: <http://www.wikidata.org/prop/direct/>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
SELECT ?{item.varname} ?{item.varname}Label"""
        for wdProp in self.properties.values():
            if naive:
                ...
                sparqlQuery+=f"\n  (SAMPLE (?{wdProp.varname}) AS ?{wdProp.varname})"
        sparqlQuery+=f"""
WHERE {{
...
"""
  ...
  OPTIONAL {{ ?{item.varname} wdt:{wdProp.pid} ?{wdProp.varname}. }}
"""                
        # close where Clause
        sparqlQuery+="""}\n"""
